...
class TickerUsESP32 {
  public:
    ...
    void setMaxIterations(uint32_t maxIterations) { _maxIterations = maxIterations; }
    ...
    void delay_ms(uint32_t milliseconds, void (*callback)(TArg), TArg arg) {
        static_assert(sizeof(TArg) <= sizeof(uint32_t),
                      "delay_ms() callback argument size must be <= 4 bytes");
        auto arg32 = (uint32_t)arg;
    ...
    void attach_us(uint64_t microseconds, void (*callback)(TArg), TArg arg) {
        static_assert(sizeof(TArg) <= sizeof(uint32_t),
                      "attach_us() callback argument size must be <= 4 bytes");
        auto arg32 = (uint32_t)arg;
